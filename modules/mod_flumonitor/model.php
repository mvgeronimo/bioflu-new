...
function getTagsCount($longitude,$latitude){
...
function getTagsCountPhil($longitude,$latitude){
		// Initialiase variables.
		$db = JFactory::getDBO();
		//$userid = $arr['user_id'];
...
function getTwitterTagsCountPhil($longitude,$latitude){
		// Initialiase variables.
		$db = JFactory::getDBO();
		//$userid = $arr['user_id'];
		//$sql = "SELECT id, ( 3959 * acos( cos( radians('12.879721') ) * cos( radians( latitude ) ) * cos( radians( longitude ) - radians('121.77401699999996') ) + sin( radians('12.879721') ) * sin(radians(latitude)) ) ) AS distance
 		//FROM `bio_twitter_reports`  WHERE `date_generated`> NOW() - INTERVAL 7 DAY HAVING `distance`< 10";
		$sql = "SELECT id, ( 3959 * acos( cos( radians('".$latitude."') ) * cos( radians( latitude ) ) * cos( radians( longitude ) - radians('".$longitude."') ) + sin( radians('".$latitude."') ) * sin(radians(latitude)) ) ) AS distance FROM `bio_twitter_reports`  WHERE `date_generated`> NOW() - INTERVAL 7 DAY HAVING `distance`< 10";
		$db->setQuery($sql);  
		$results = $db->loadObjectList();
		try{
			$result = $db->loadAssocList();
...
function get_default_place(){
    $db = JFactory::getDBO();
    $sql = "SELECT name FROM bio_maps_default";
