...
function nfm_count(){
		...
		$flu_reports_tag = count(getTagsCountPhil($long,$lat));
		$flu_reports_twitter = count(getTwitterTagsCountPhil($long,$lat));
		$flu_reports = $flu_reports_tag + $flu_reports_twitter;

		$nfr_reports_tag = count(getTagsCountPhil("121.77401699999996","12.879721"));
		$nfr_reports_twitter = count(getTwitterTagsCountPhil("121.77401699999996","12.879721"));
		$nfr_reports = $nfr_reports_tag + $nfr_reports_twitter;

		$drugStores = count(getDrugstores($long,$lat));
		$default_place = get_default_place();
// ,'flucount' => $data
		echo json_encode(array('location' => $default_place,'drugStores' => $drugStores, 'flu_reports' => $flu_reports, 'nfr_reports' => $nfr_reports));		
}
