...
if ($_POST['finish'] == '0000-00-00 00:00:00' || $_POST['finish'] == '') {
	...
	$data = array(
		...
		'alias' => $alias,
		'state' => $_POST['state'],
		'catid' => '8', //category id ex. articles
	...
	$tbl_name = '#__menu';
	$data_menu = json_decode(json_encode($data_menu), FALSE);
	save_data($data_menu,$tbl_name);
}
...
function update_articles(){
	...
	$link = 'index.php?option=com_content&view=article&id='.$_POST['id'];
	$data_menu = array(
		'menutype' => 'articles',
		'title' => $_POST['title'],
		'alias' => $alias,
	...
	$tbl_name = '#__menu';
	$data_menu = json_decode(json_encode($data_menu), FALSE);
	update_data($data_menu,$tbl_name,'link');
...
function get_twitter_reports(){
	$limit = $_POST['limit'];
	$offset = ($_POST['offset']-1)* $limit;
	$result = gettwitterreports($limit,$offset);
$x=1;
$htm = "";
$htm .= '<table class = "table-bordered">';
$htm .= '<thead>';
...
function get_twitter_reports_count() {
	...
	$limit = '9999999';
	$offset = '0';
	$data = count(gettwitterreports($limit,$offset));
	$page_count = ceil($data/$per_page);
	echo $page_count;
}

function get_twitter_reports_export(){
	$limit = 0;
	$offset = 0;
	$result = gettwitterreports($limit,$offset);
$x=1;
