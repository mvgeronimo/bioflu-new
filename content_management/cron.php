...
// after query tweets, add geocode
ts_add_geocode();
...
function ts_push_tweets($tweets, $keyword, $datefrom, $dateto){
    ...
    if($tweets){
        foreach ($tweets as $tweet) {
            $ts_push_tweets['sn'] = (string)$tweet->id;
    ...
    if(count($tweets) >= 99){
        $str_max_id = (string)$tweets[99]->id;
        $qtweets = ts_query_tweets_1($keyword, $datefrom, $dateto, $str_max_id);
        if($tweets->errors[0]->code == 88){
            $tweets = ts_query_tweets_2($keyword, $datefrom, $dateto, $str_max_id);
            if($tweets->errors[0]->code == 88){
                $tweets = ts_query_tweets_3($keyword, $datefrom, $dateto, $str_max_id);
            }
        }
...
function ts_add_geocode(){
    $curr_date = date("Y-m-d");
    $tb_tweets = mysql_query("SELECT * FROM bio_twitter_reports WHERE date_generated='{$curr_date}' AND (latitude = 'X' OR latitude = '') AND (longitude = 'X' OR longitude = '') AND address != '' ") or die(mysql_error());
    $ctr = 1;
    while ($row = mysql_fetch_assoc($tb_tweets)) { 
        $geocode = get_geocode($row['address']);
        $geocode = explode("#", $geocode);
        echo $ctr++.'.) '.$row['address'].' - longitude:'.$geocode[0].' - latitude:'.$geocode[1].' - '.$row['sn'].'</br>';
        $tb_tweet = "UPDATE bio_twitter_reports SET longitude='{$geocode[0]}', latitude='{$geocode[1]}' WHERE sn='{$row['sn']}'";
        mysql_query($tb_tweet) or die(mysql_error());
    } // end while
} // ts_add_geocode
