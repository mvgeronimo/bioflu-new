        ...
        data:{id:test,limit:limit, table:table, field:field},
          ...
          $.each(obj, function(index, row){    
               ...
               // $('.selected-image-1').html('<img class = "image1" src="../'+row.video+'"><input style = "display:none" class = "image1 inputs" value="'+row.video+'"/>');  
                $('.selected-image-2').html('<span class = "glyphicon glyphicon-remove btn-remove-image"></span><img class = "image2" src="../'+row.image+'"><input style = "display:none" class = "image2 inputs" value="'+row.image+'"/>');
                $('.mytitle').val(row.article_title);
                $('.mytags').val(row.tags);
                $('.mytext').html(row.intro_text).blur();
                $('#datepicker').val(row.date_published);
          });
     });      
}
...
function validateFields() {
 ...
 $(document).on('click', '.update', function(){
  ...
  $.ajax({
          ...
          data:{id:id, mytitle:mytitle, mytext:mytext, image2:image2, mytags:mytags, date_published:date_published , action: 'update'},
        }).done( function(data){
