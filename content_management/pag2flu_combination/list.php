...
$(document).ready(function() {
  ...
  function last(){
    ...
    $.ajax({
        ...
        data:{limit:limit, offset:offset, search:search,table:table,order_by:order_by},
      }).done( function(data){
          ...
          $.each(obj, function(index, row){ 
            if(row.state==1){
              ...
              htm+="<td><a class='edit' data-status='"+row.state+"' id='"+row.id+"' title='edit'><d style='color:#3071a9'>"+row.name+"</d></a></td>";
              htm+="<td><img src='../"+row.image+"' class='img-responsive img-banner'/></td>";
              htm+="<td>"+status+"</td>";
              htm+="<td><a class='edit' data-status='"+row.state+"' id='"+row.id+"' title='edit'><span class='glyphicon glyphicon-pencil'></span></a></td>";
              htm+="</tr>";
            });
          $('.listdata tbody').html('');
  ...
  function get_pagination(page_num,limit,search){
    $.ajax({
        type: 'Post',
        url: 'dashboard.php?function=get_banner_count',
