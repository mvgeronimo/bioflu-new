...
button();
 ...
 function button(){
  ...
  function last(){
    ...
    $.ajax({
        type: 'Post',
        url: 'dashboard.php?function=get_article_featured',
        data:{limit:limit, offset:offset},
          ...
          $.each(obj, function(index, row){ 
            if(row.state==1){
              ...
              htm+="<td>"+status+"</td>";
              htm+="<td align='center'>";
              htm+="<img class = 'loader_"+row.id+"' style='display:none' src = 'assets/img/loader-blue.gif' />";
              htm+="<p class = 'ordering ordering_"+row.id+"' data-order='"+row.id+"' style='color:#3071a9; cursor:pointer'>"+row.ordering+"</p>";
              htm+="<input type = 'text' data-order='"+row.id+"' class = 'order-input order-input_"+row.id+"' value ='"+row.ordering+"' style = 'display:none; text-align:center'>";
              htm+="</td>";
  ...
  function get_pagination(page_num,limit){
    $.ajax({
        type: 'Post',
        url: 'dashboard.php?function=get_article_featured_count',
        data:{limit:limit},
      }).done( function(data){
          var htm = '';
  ...
  $(document).on('click', '.edit',function(){
    ...
    $.ajax({
        type: 'Post',
        url: 'dashboard.php?function=edit_article_featured',
        data:{id:id},
      }).done( function(data){
              var obj = JSON.parse(data);
              var htm = '';
          $.each(obj, function(index, row){ 
              ...
              htm+="<button class = 'btn-select-thumb-image'>SELECT IMAGE</button> (Image size: 395x305px)";
              ...
              htm+="<input class = 'edit_title_text inputs' type = 'text' value = '"+row.title+"' disabled>";
              htm+="<button class = 'btn-select-article'>SELECT</button>";
              htm+="<span class = 'er-msg msg-prompt'> * Please select article...</span>";
              htm+="</div>";
              htm+="</div>";
...
$(document).on('click', '.btn-update', function(){
  ...
  if(counter == 0){
     $.ajax({
        type: 'Post',
        url: 'dashboard.php?function=update_article_featured',
      ...
      $('.select').each(function() {                
          if (this.checked==true) {
             ...
             var id = $(this).attr('data-id');
              ...
              $.ajax({
                  type: 'Post',
                  url: 'dashboard.php?function=update_article_featured_status',
...
$('.btn-addnew').click(function(){
    ...
    $('.addnew-data').show();
              ...
              htm+="<input class = 'edit_title_text' type = 'text' value = '' disabled>";
              htm+="<button class = 'btn-select-article'>SELECT</button>";
...
$(document).on('click', '.btn-saved', function(){
  ...
  var state = $('.status').val();
   ...
   var counter = 0;
    $('.inputs').each(function(){
     ...
     $.ajax({
        type: 'Post',
        url: 'dashboard.php?function=get_max_order',
       data:{},
      }).done( function(data){
              ...
              var ordering = parseInt(data) + parseInt(1);
  ...
  if(counter == 0){
    $.ajax({
        type: 'Post',
        url: 'dashboard.php?function=saved_article_featured',
       data:{image_path: image_path, article_id: article_id, state: state, ordering:ordering},
      }).done( function(data){
...
$(document).on('click', '.ordering', function(){
  ...
  $('.order-input_'+id).show(); 
  $('.ordering_'+id).hide();
});

$(document).on('change', '.order-input', function(){
  var order_number = $(this).val();
  var data_id = $(this).data('order');

  $('.order-input_'+data_id).hide(); 
  
 
  $('.order-input_'+data_id).val(order_number); 
  $('.loader_'+data_id).show();
   $.ajax({
        type: 'Post',
        url: 'dashboard.php?function=update_article_ordering',
        data:{id: data_id, ordering:order_number},
      }).done( function(data){
        setTimeout(function(){
        $('.loader_'+data_id).hide();
         $('.ordering_'+data_id).html(order_number);
         $('.ordering_'+data_id).show();
        }, 3000);
     });   
