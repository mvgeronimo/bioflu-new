...
button();
 ...
 function button(){
  ...
  function last(){
    ...
    $.ajax({
        type: 'Post',
        url: 'dashboard.php?function=get_menu_settings',
        data:{limit:limit, offset:offset},
          ...
          $.each(obj, function(index, row){ 
            if(row.published==1){
              ...
              htm+="<td>"+status+"</td>";
              htm+="<td><a class='edit' data-status='"+row.published+"' id='"+row.id+"' title='edit'><span class='glyphicon glyphicon-pencil'></span></td>";
              htm+="</tr>";
            });
          $('.listdata tbody').html('');
  ...
  function get_pagination(page_num,limit){
    $.ajax({
        type: 'Post',
        url: 'dashboard.php?function=get_menu_settings_count',
        data:{limit:limit},
      }).done( function(data){
          var htm = '';
  ...
  $(document).on('click', '.edit',function(){
    ...
    $.ajax({
        type: 'Post',
        url: 'dashboard.php?function=edit_menu',
        data:{id:id},
      }).done( function(data){
              var obj = JSON.parse(data);
              var htm = '';
          $.each(obj, function(index, row){ 
              ...
              htm+="<label>Menu Title:</label>";
              htm+="</div>";
              htm+="<div class = 'col-md-10'>";
              htm+="<input class = 'edit_latitude_text inputs' type = 'text' value = '"+row.title+"'>";
              ...
              htm+="<div class = 'col-md-12 pad-5'>";
              htm+="<div class = 'col-md-2 '>";
              htm+="<label>Select Page: </label>";
              htm+="</div>";
              htm+="<div class = 'col-md-10'>";
              htm+="<div class = 'col-md-3 pad-0'>";
              htm+="<input class = 'edit_longitude_text inputs fullwidth' type = 'text' value = '"+row.alias+"' disabled>";
              htm+="</div>";
              htm+="<div class = 'col-md-3 pad-0'>";
              htm+="<button class = 'btn btn-default btn-standard btn-select-article'  data-id = '"+row.link+"'>SELECT</button>";
              htm+="</div>";
              ...
              htm+="<option value = '"+row.published+"'>"+status+"</option>";
              ...
              htm+="</div>";
              htm+="<div class = 'col-md-10'>";
              htm+="<button style = 'margin-right:5px;' class = 'btn btn-primary btn-min btn-update-hospital'  data-id = '"+row.hospital_id+"'>Update</button>";
              htm+="<button class = 'btn btn-default btn-min btn-cancel'  data-id = '"+row.hospital_id+"'>Cancel</button>";
              htm+="</div>";
              htm+="</div>";
