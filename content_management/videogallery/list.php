...
$(document).ready(function() {
  ...
  function last(){
    ...
    $.ajax({
        type: 'Post',
        url: 'dashboard.php?function=get_videogallery',
        data:{limit:limit, offset:offset},
          ...
          $.each(obj, function(index, row){ 
            if(row.status==1){
              ...
              // htm+='<td align="center"><video width="100%" controls><source src="../'+row.video+'" type="video/mp4"></video></td>';
              htm+="<td>"+row.title+"</td>";
              htm+="<td>"+row.path+"</td>";
              htm+="<td><img src='../images/bioflu/"+row.thumbnail+"' height='70px'></td>";
              htm+="<td>"+status+"</td>";
              if(row.is_featured == 1){
                htm+="<td><span class='glyphicon glyphicon-star'></span></td>";
              }else{
                htm+="<td></td>";
              }
              htm+="<td><a class='edit' data-status='"+row.status+"' id='"+row.id+"' title='edit'><span class='glyphicon glyphicon-pencil'></span></td>";
              htm+="</tr>";
            });
          $('.listdata tbody').html('');
  ...
  function get_pagination(page_num,limit){
    $.ajax({
        type: 'Post',
        url: 'dashboard.php?function=get_videogallery_count',
        data:{limit:limit},
      }).done( function(data){
          var htm = '';
      ...
      $('.select').each(function() {                
          if (this.checked==true) {
             ...
             var id = $(this).attr('data-id');
              ...
              $.ajax({
                  type: 'Post',
                  url: 'dashboard.php?function=update_videogallery_status',
