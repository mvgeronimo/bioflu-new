...
$data = get_twitter_interjection();

foreach ($data as $key => $value) {
	$data2[$key] = $value['symptom'];
}

//data2 same data as keywords
...
function get_twitter_interjection() {
	$data_array = array();
	$results = mysql_query("SELECT CONCAT(symptom1,' AND ',symptom2) as symptom  FROM bio_twitter_interjection order by id ");
...
function ts_save_tweets($arr_tweets = array()){   
    if($arr_tweets){
        ...
        $tb_sn          = number_format($tweet['sn'],0,'','');
        ...
        $address = str_replace(" ", "+", $value['location']);
        ...
        if($tweet['latitude']==''){
            ...
            $latitude = mysql_real_escape_string($tweet['latitude']);
            $longitude = mysql_real_escape_string($tweet['longitude']);
        }

        $tb_longitude   = $latitude;
        $tb_latitude    = $longitude;
