...
function getcomments($limit,$offset){
	...
	$query
		...
		->join('LEFT', $db->quoteName('#__flunew_articles', 'b') . ' ON (' . $db->quoteName('a.id') . ' = ' . $db->quoteName('b.id') . ')')
...
function check_combination($flu_two,$flu_one){
	...
	$query
		...
		->where($db->quoteName('flu_one') . '= "'.$flu_one.'" ',"AND")

		->where($db->quoteName('flu_two') . '= "'.$flu_two.'" ',"AND")
