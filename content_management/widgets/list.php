...
$(document).ready(function() {
  ...
  function last(){
    ...
    $.ajax({
        type: 'Post',
        url: 'dashboard.php?function=get_ads',
        data:{limit:limit, offset:offset},
          ...
          $.each(obj, function(index, row){ 
            if(row.status==1){
              ...
              htm+="<td>"+row.title+"</td>";
              htm+="<td>"+status+"</td>";
              htm+="<td><a class='edit' id='"+row.id+"' title='edit'><span class='glyphicon glyphicon-pencil'></span></td>";
              htm+="</tr>";
            });
          $('.listdata tbody').html('');
  ...
  function get_pagination(page_num,limit){
    $.ajax({
        type: 'Post',
        url: 'dashboard.php?function=get_ads_count',
        data:{limit:limit},
      }).done( function(data){
          var htm = '';
...
$('.btn-navigation').addClass('col-md-6');
